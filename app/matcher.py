...
# Default DB path relative to repo
DEFAULT_DB_PATH = str(Path("data/scryfall_db/cards.db"))

def load_db_phashes(db_path: str = DEFAULT_DB_PATH) -> List[Tuple[str,str,str,str]]:
    """
    Return list of rows (id, name, phash, desc_file) from cards table where phash exists.
    """
...
def verify_candidate_orb(query_bgr: np.ndarray, candidate_npz_path: Path, min_good: int = DESCRIPTOR_MIN_GOOD_MATCHES, ratio: float = DESCRIPTOR_RATIO_TEST) -> Tuple[bool,int]:
    """
    Verify a candidate by matching ORB descriptors.
    Returns (is_verified, good_matches_count).
    Expects candidate_npz_path to be a .npz file containing an array under key "des".
    """
    orb = cv2.ORB_create(1000)
    qgray = cv2.cvtColor(query_bgr, cv2.COLOR_BGR2GRAY)
    _, qd = orb.detectAndCompute(qgray, None)
